...
in vec3 textureVector; //texCoords
...
{
	//Ambient
	vec3 ambient = light.ambient * vec3(texture(material.ambient * light.ambient, textureVector));

	//diffuse
	vec3 norm = normalize(vNormalEyeSpace);
	//vec3 lightPositionEyeSpace = normalize(light.position - vPositionEyeSpace);
	vec3 ViewPos = (View * vec3(light.position)).xyz;
	vec3 LightDir = normalize(ViewPos - vPositionEyeSpace); //L
	float diff = max(dot(norm, lightPositionEyeSpace),0.0);				 //N
	vec3 diffuse = light.diffuse * diff * vec3(texture(material.diffuse, textureVector));

	//Specular
	vec3 V = normalize(View.position-vPositionEyeSpace);
	vec3 R = reflect(-LightDir, norm);
	float spec = pow(max(dot(V, R), 0.0),material.shininess);
	vec3 specular = light.specular * spec * vec3(texture(material.specular,textureVector))
	...
	float distance = length(light.position - vPositionEyeSpace);	// C�lculo da dist�ncia entre o ponto de luz e o v�rtice
